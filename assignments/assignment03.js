...
//add elem[19] as a parameter for the add1 function to update total
elem[1].children[4].children[0].onclick = function() {
	add1(elem[1], elem[19]);
};
//when the minus button is clicked the sub1 function is passed the current hole's value and the total so it can be updated
elem[1].children[4].children[1].onclick = function() {
	sub1(elem[1], elem[19]);
};
//when the C button is clicked the current row/hole is cleared and updates the value for that hole and the total 
elem[1].children[4].children[2].onclick = function() {
	clear(elem[1], elem[19]);
};
elem[2] = document.getElementById("2");
// assign a function to the + button
//add elem[19] as a parameter so the total can be updated
